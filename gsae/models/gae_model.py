...
class GAE(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if hparams.gnn_type == 'gcn':
            ...
            self.gcn_2 = DenseGCNConv(hparams.hidden_dim, self.prepool_dim)

        elif hparams.gnn_type == 'sage':
            self.gcn_1 = gnn_modules.DenseSAGEConv(hparams.input_dim, hparams.hidden_dim)
            self.gcn_2 = gnn_modules.DenseSAGEConv(hparams.hidden_dim, self.prepool_dim)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, log_losses = self.loss_multi_gnnAE(recon_x=adj_hat,x=adj, 
                                                y_pred=y_hat,y=y,
